...
> Place this file at `rover-autoresponder/CLAUDE.md` (drop the `rover-autoresponder-` prefix
> the download used).
...
Three systemd **user** services in `systemd/` (linger enabled). Read the unit files for the
authoritative `ExecStart` — resolved so far:

- `rover-sms.service` → the SMS pipeline (`sms_main.py`): ingest, parse, draft, approve-send.
  **This service owns the Telegram poller.**
- `rover-email-fallback.service` → email pipeline in **`EMAIL_MODE=fallback`**: ingest-only
  for confirmation emails, modification emails, and truncation recovery; **no drafting, no
  Telegram poll** (so it can't fight the SMS service's poller). *"fallback" is the mode name —
  the data it feeds is load-bearing (see "Email pipeline" above), not optional.*
- `rover-autoresponder.service` → the original full email-mode service (pre-SMS-cutover).
  **Verify whether it's still enabled** or has been superseded by the SMS service post-cutover.
...
## Build status to verify against the code
...
- Calendar: **C4 (modification/cancellation) and C5 (private bookings) appear built** — the
  full command set and modification/cancellation handling exist and were debugged. **C6 (48h
  reminders) unconfirmed** — check before assuming.
- `rover_autoresponder.db.bootalert`: purpose unclear from the docs/chat — verify it isn't a
  stray committed DB copy, and that it's gitignored.